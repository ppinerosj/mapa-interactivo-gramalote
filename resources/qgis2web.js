...
map.getView().fit([-8124702.888762, 872776.958731, -8076694.659580, 897363.086324], map.getSize());
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    var popupText = '<div style="background:#f9f9f9; border-radius:8px; padding:12px; box-shadow:inset 0 0 4px rgba(0,0,0,0.1); font-size:13.8px; line-height:1.4;">';
    ...
    for (var i = 0; i < currentFeatureKeys.length; i++) {
        ...
        var popupField = '';
        var fieldLabel = layer.get('fieldLabels')[key];
        var fieldImage = layer.get('fieldImages')[key];
        var fieldAlias = layer.get('fieldAliases')[key];
        var fieldValue = currentFeature.get(key);

        if (fieldLabel === "hidden field") {
            continue;
        } else if (fieldLabel === "inline label - visible with data" && fieldValue == null) {
            continue;
        }

        // Nombre del lugar en destacado
        if (fieldAlias.toLowerCase() === 'nombre') {
            popupText += '<div style="font-size:16.5px; font-weight:bold; color:#222; margin-bottom:8px;">' + fieldValue + '</div>';
            continue;
        }

        // Campo normal con alias y contenido
        popupField += '<div style="margin-bottom:5px;">';
        popupField += '<span style="font-weight:600; color:#333;">' + fieldAlias + ': </span>';

        if ((fieldLabel === "header label - visible with data" && fieldValue == null)) {
            continue;
        }

        if (key.toLowerCase() === "foto" && fieldValue && /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(fieldValue)) {
            popupField += '<br /><img src="' + fieldValue + '" style="max-width:100%; max-height:220px; margin-top:5px; border-radius:6px;" />';
        } else {
            popupField += (fieldValue != null ? autolinker.link(fieldValue.toString()) : '');
        }
...
function onSingleClickFeatures(evt) {
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
    if (!layer || ['Límite municipal', 'Drenajes', 'Vías'].includes(layer.get('popuplayertitle'))) {
        return; // Omitir estas capas en los pop-ups
    }
        if (layer && feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") === undefined)) {
            var doPopup = false;
...
var Title = new ol.control.Control({
    element: (() => {
        ...
        titleElement.innerHTML = '<h2 class="project-title">🗺️ Mapa turístico interactivo de Gramalote</h2>';
        return titleElement;
    })(),
